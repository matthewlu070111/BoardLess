...
function OwnerUsers() {
  const { siteMode } = useAuth();
  const [users, setUsers] = useState<Json[] | null>(null); const [link, setLink] = useState(""); const [inviteRole, setInviteRole] = useState<"user" | "admin">("admin"); const [planUser, setPlanUser] = useState<Json | null>(null); const [entitlements, setEntitlements] = useState<Json[] | null>(null); const [error, setError] = useState("");
  const [danger, setDanger] = useState<DangerAction | null>(null);
  const load = async () => { const result = await api<{ users: Json[] }>("/api/owner/users"); setUsers(result.users); }; useEffect(() => { void load(); }, []);
  const invite = async (role: "user" | "admin") => { const result = await api<Json>("/api/owner/invitations", { method: "POST", body: JSON.stringify({ role, expiresHours: 72 }) }); setInviteRole(role); setLink(result.url); await navigator.clipboard.writeText(result.url); };
  ...
  if (!users) return <Loading />;
  return <Page title="账号管理" description="管理账号状态与基础财务信息；节点权限在专用页面配置" action={<div className="quick-actions"><button className="button primary" onClick={() => invite("user")}>{inviteActionLabel("user")}</button><button className="button" onClick={() => invite("admin")}>{inviteActionLabel("admin")}</button></div>}>
    {link && <Notice tone="success">{inviteSuccessMessage(inviteRole)}：<code className="token">{link}</code></Notice>}
