...
function OwnerPlans() {
  ...
  return <Page title="套餐编排" description="全站只能启用一种节点授权方式" action={mode === "plan" ? <button className="button primary" onClick={() => openPlan()}>＋ 新建套餐</button> : <Link className="button primary" to="/owner/users">前往分配用户节点</Link>}>
    <section className="panel authorization-mode"><div><p className="eyebrow">节点授权模式</p><h2>{mode === "plan" ? "按套餐授权" : "按用户直接授权"}</h2><p className="muted">{mode === "plan" ? "用户获得当前套餐中配置的节点，用户直配关系不会生效。" : "用户只获得账号管理中直接分配的节点，套餐节点关系不会生效。"}</p></div><div className="mode-switch"><button className={`button ${mode === "plan" ? "primary" : ""}`} onClick={() => switchMode("plan")}>按套餐授权</button><button className={`button ${mode === "user" ? "primary" : ""}`} onClick={() => switchMode("user")}>按用户授权</button></div></section>
