...
function Subscription() {
  ...
  return <Page title="订阅" description="按客户端复制对应格式；订阅链接等同密码，请勿公开" action={<button className="button danger-ghost" onClick={rotate}>轮换链接</button>}>
    ...
    {!data.active && <Notice tone="danger">当前没有有效套餐，订阅暂不可用。</Notice>}
    <div className="card-grid targets">{[
      ["clash", "Clash Meta", "YAML"], ["singbox", "sing-box", "JSON"], ["surge", "Surge", "SS / Trojan"], ["base64", "通用订阅", "URI Base64"],
    ...
    <section className="panel"><h2>兼容说明</h2><p className="muted">Clash Meta 与 sing-box 输出全部六类协议。Surge 仅输出其稳定支持的 Shadowsocks 和 Trojan 节点；不兼容节点会通过响应头标明，不会生成伪配置。</p></section>
  </Page>;
}
...
function OwnerPlans() {
  ...
  const [form, setForm] = useState<Json>({ name: "", description: "", price: "", durationDays: 30, quotaGb: 100, nodePoolPercent: 0, nodeIds: [] });
  ...
  const save = async (event: FormEvent) => { event.preventDefault(); setError(""); try { await api("/api/owner/plans", { method: "POST", body: JSON.stringify({ name: form.name, description: form.description, priceCents: Math.round(Number(form.price) * 100), durationDays: Number(form.durationDays), quotaBytes: Math.round(Number(form.quotaGb) * 1024 ** 3), nodePoolBps: Math.round(Number(form.nodePoolPercent) * 100), nodeIds: form.nodeIds }) }); setShow(false); await load(); } catch (e) { setError(e instanceof Error ? e.message : "保存失败"); } };
  if (!plans) return <Loading />;
  return <Page title="套餐编排" description="只有已审核节点可以加入套餐" action={<button className="button primary" onClick={() => setShow(true)}>＋ 新建套餐</button>}>
    <div className="card-grid plans compact">{plans.map((plan) => <article className="plan-card" key={plan.id}><div className="row-between"><Badge value={plan.status} /><span>{plan.nodeIds.length} 节点</span></div><h2>{plan.name}</h2><div className="price"><span>¥</span>{(plan.price_cents / 100).toFixed(2)}<small> / {plan.duration_days} 天</small></div><ul><li>{bytes(plan.quota_bytes)} / 自然月</li><li>节点收益池 {(plan.node_pool_bps / 100).toFixed(2)}%</li></ul></article>)}</div>
    {!plans.length && <Empty>暂无套餐</Empty>}
    {show && <div className="modal-layer"><form className="modal" onSubmit={save}><button type="button" className="modal-close" onClick={() => setShow(false)}>×</button><p className="eyebrow">套餐配置</p><h2>新建套餐</h2><div className="form-grid"><Field label="套餐名称"><input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></Field><Field label="价格（元）"><input type="number" min="0" step="0.01" required value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} /></Field><Field label="有效期（天）"><input type="number" min="1" required value={form.durationDays} onChange={(e) => setForm({ ...form, durationDays: e.target.value })} /></Field><Field label="每月流量（GB）"><input type="number" min="1" required value={form.quotaGb} onChange={(e) => setForm({ ...form, quotaGb: e.target.value })} /></Field><Field label="节点收益池（%）"><input type="number" min="0" max="100" step="0.01" value={form.nodePoolPercent} onChange={(e) => setForm({ ...form, nodePoolPercent: e.target.value })} /></Field><Field label="说明"><input value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} /></Field></div><fieldset className="node-picker"><legend>选择节点</legend>{nodes.map((node) => <label key={node.id}><input type="checkbox" checked={form.nodeIds.includes(node.id)} onChange={(e) => setForm({ ...form, nodeIds: e.target.checked ? [...form.nodeIds, node.id] : form.nodeIds.filter((id: string) => id !== node.id) })} /> <span>{node.name}</span><small>{node.protocol} · {node.owner_email}</small></label>)}</fieldset>{error && <Notice tone="danger">{error}</Notice>}<div className="modal-actions"><button type="button" className="button" onClick={() => setShow(false)}>取消</button><button className="button primary">创建套餐</button></div></form></div>}
  </Page>;
}

function OwnerUsers() {
  const [users, setUsers] = useState<Json[] | null>(null); const [link, setLink] = useState("");
  const load = () => api<{ users: Json[] }>("/api/owner/users").then((r) => setUsers(r.users)); useEffect(() => { void load(); }, []);
